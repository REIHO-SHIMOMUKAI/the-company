...
{
    // store method / inserting user to do
    public function store($request)
    {
        ...
        // conn is come from Database class
    ...
    public function login($request)
    {
        ...
        {
            //success
            //check if username exists in do
            if($result->num_rows == 1)
            {
                ...
                if(password_verify($password,$user['password']))
                {
                    ...
                    // adding session variables and values
                    $_SESSION['id'] = $user['id'];
    ...
    {
        //edit-user.phpでfirstnameとlastnameを表示するために呼ぶ関数
        //login()で$_SESSION[first_name]としてとってくるのもいいが
        //データベースからダイレクトにデータを取ってきたほうがセキュリティが高い
        $id = $_SESSION['id'];
    ...
    {
        //start settion to get session variables
        session_start();
        $id = $_SESSION['id'];
        ...
        $last_name = $request['last_name'];
        $username = $request['username'];
        $photo = $files['photo']['name'];//name of file
